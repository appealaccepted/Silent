	...
	if self == Mouse and (Index == "Hit") then 
		...
		if SilentAim then
			...
			for _, v in pairs(Players:GetPlayers()) do 
				if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild("HumanoidRootPart") and v.Character:FindFirstChild("UpperTorso") and v.Character:FindFirstChild("Humanoid").Health > 0 then
		...
		if Targete ~= nil and Targete[Options.Torso] and Targete:FindFirstChild("Humanoid").Health > 0 then
			if SilentAim then
				local ShootThis = Targete[Options.Torso] --and Targete[Options.Upper] --or Targete[Options.Upper] --and Targete[Options.Head] and Targete[Options.Upper]
